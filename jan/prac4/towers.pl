...
satVariable( towerVillage(V) ):- village(V). % there's a tower in village V
...
noTowersOutsideVillage:-
    position(I,J), \+posVillage(_,I,J), writeClause([-towerPos(I,J)]), fail.
noTowersOutsideVillage.
...
checkVillageTowers1:-
    posVillage(V,I,J), writeClause([-towerPos(I,J), towerVillage(V)]), fail.
checkVillageTowers1.

checkVillageTowers2:-
    village(V), findall(towerPos(I,J), posVillage(V,I,J), Lits), atLeast(1, [-towerVillage(V)|Lits]), fail.
checkVillageTowers2.
...
eachSignificantVillageOneTower:-
    significantVillage(V), writeClause([towerVillage(V)]), fail.
eachSignificantVillageOneTower.
...
costOfThisSolution(M,Cost):- findall(village(V), member(towerVillage(V), M), L), length(L, Cost).
