...
satVariable( free(G,H) ):- gangster(G), hour(H).
...
writeClauses(MaxConsecutiveHours):-
    checkFree,
    noGangsterHourTwoTasks,
    noGangsterConsecutiveDifferentTasks,
    noGangsterHourBlocked,
    allHourTaskEnoughGangsters,
    noGangsterWorkingKConsecutiveHours(MaxConsecutiveHours),
    true,!.
writeClauses(_):- told, nl, write('writeClauses failed!'), nl,nl, halt.

checkFree:-
    task(T), hour(H), gangster(G), writeClause([-free(G,H), -does(G,T,H)]), fail.
checkFree.

noGangsterHourTwoTasks:-
    gangster(G), hour(H), findall(does(G,T,H), task(T), Lits), atMost(1, Lits), fail.
noGangsterHourTwoTasks.

noGangsterConsecutiveDifferentTasks:-
    gangster(G), task(T1), task(T2), hour(H1), hour(H2), T1 \= T2, H2 is H1+1, writeClause([-does(G,T1,H1), -does(G,T2,H2)]), fail.
noGangsterConsecutiveDifferentTasks.

noGangsterHourBlocked:-
    blocked(G,H), writeClause([free(G,H)]), fail.
noGangsterHourBlocked.

allHourTaskEnoughGangsters:-
    needed(T,H,N), findall(does(G,T,H), gangster(G), Lits), atLeast(N, Lits), fail.
allHourTaskEnoughGangsters.

noGangsterWorkingKConsecutiveHours(MaxConsecutiveHours):-
    gangster(G), hour(H0), hour(H1), H1 is H0+MaxConsecutiveHours, findall(free(G,H), between(H0,H1,H), Lits), atLeast(1, Lits), fail.
noGangsterWorkingKConsecutiveHours(_).
...
gangsterWorkingKConsecutiveHours(M,K):- gangster(G), task(T), hour(H0), hour(H1), H1 >= H0, K is H1-H0+1, forall(between(H0,H1,H), member(does(G,T,H), M)).

costOfThisSolution(M,Cost):- findall(K, gangsterWorkingKConsecutiveHours(M,K), L), max_list(L, Cost).
