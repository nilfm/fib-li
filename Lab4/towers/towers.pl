...
posWatchesVillage(V,I,J):- position(I,J), colVillage(V,J).            % position (I,J) watches village V
...
noTowerOutsideVillage:-
    position(I, J),
    % \+ succeeds if its argument is not provable
    % In this case, if there is no village in the position (I, J)
    \+posVillage(_, I, J),
    writeClause([-towerPos(I, J)]),
    fail.
noTowerOutsideVillage.
...
% Guarantee that if there is a tower in a village then towerVillage(V) is true
bindVariables1:-
    posVillage(V, I, J),
    % either there is no tower on (I, J) or the village V has a tower
    writeClause([-towerPos(I, J), towerVillage(V)]),
    fail.
bindVariables1.

% Guarantee that if there is no tower in a village then towerVillage(V) is false
bindVariables2:-
    village(V),
    findall(towerPos(I, J), posVillage(V, I, J), Lits),
    atLeast(1, [-towerVillage(V)|Lits]),
    fail.
bindVariables2.
...
costOfThisSolution(M,Cost):-
    findall(village(V), member(towerVillage(V), M), Lits),
    length(Lits, Cost).
